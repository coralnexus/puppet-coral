...
module Resource
  ...
  def self.translate(type_name, resources)    
    resources.each do |name, data|
      resources[name]['before']    = translate_resource_refs(type_name, data['before']) if data.has_key?('before')
      resources[name]['notify']    = translate_resource_refs(type_name, data['notify']) if data.has_key?('notify')
      resources[name]['require']   = translate_resource_refs(type_name, data['require']) if data.has_key?('require')       
      resources[name]['subscribe'] = translate_resource_refs(type_name, data['subscribe']) if data.has_key?('subscribe')      
    end
    return resources
  end
  ...
  def self.translate_resource_refs(type_name, resource_refs)
